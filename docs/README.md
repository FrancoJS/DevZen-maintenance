...
- Los tickets implementan creación, listado, detalle, edición propia `NEW`, asignación inicial transaccional, inicio, registro técnico en `IN_PROGRESS` y resolución con liberación atómica del técnico. El técnico puede consultar sus mantenciones históricas liberadas. Cierre y congelamiento permanecen pendientes; el detalle vigente se mantiene en [IMPLEMENTATION_CHECKLIST.md](IMPLEMENTATION_CHECKLIST.md).
